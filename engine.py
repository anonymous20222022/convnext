...
class NanDetector:
    ...
    def __exit__(self, exc_type, exc_value, exc_traceback):
        ...
        if len(gradients) > 0:
            print("Detected nan/inf grad norm, dumping norms...")
            print(f"norms: {norm}")
...
def train_step_till_norm(samples, targets, model, criterion, optimizer, data_iter_step, update_freq, loss_scaler, max_norm):
    model.train(True)
...
def train_step_till_loss(samples, targets, model, criterion, optimizer, data_iter_step, update_freq, loss_scaler, max_norm):
    print("Calling train_step_till_Loss function...")
    if loss_scaler is None:
        loss, output = train_class_batch(
            model, samples, targets, criterion)
    else:
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
                    ...
                    num_training_steps_per_epoch=None, update_freq=None, repeat_on_nan=False):
    model.train(True)
    ...
    for data_iter_step, (samples, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if repeat_on_nan:
            i_repeat = 0
            while (not math.isfinite(loss_value)):
                print("Loss is {} at iteration {}, trying to repeat forwarding \
                     {}th time".format(loss_value, data_iter_step, i_repeat))
                if i_repeat == 10:
                    print("Max NAN times reached, will break here")
                ...
                loss, output = train_step_till_loss(
                    samples, 
                    targets, 
                    model, 
                    criterion, 
                ...
                # assert(math.isfinite(loss_value))
                loss_value = loss.item()
                print("New Loss is {}".format(loss_value))
        else:
